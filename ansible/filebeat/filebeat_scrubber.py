...
# from https://github.com/barqshasbite/filebeat-scrubber
import argparse
import datetime
import json
import logging
...
def _regex(value: str):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("Invalid filter regex provided. %s" % error.msg)
...
def _get_age(timestamp: str):
    ...
    try:
        ...
        date_object = datetime.datetime.strptime(timestamp[:-4], "%Y-%m-%dT%H:%M:%S.%f")
    return (now - date_object).total_seconds()


def _read_registry_file(args: argparse.Namespace) -> List[Dict]:
    """Read the contents of the registry JSON file. This also checks for the existence of
    an active.dat file which specifies another registry JSON file.
    ...
    if args.type:
        data = [entry for entry in data if entry["v"].get("type") in args.type]
    if args.age:
        data = [entry for entry in data if _get_age(entry["v"]["timestamp"]) >= args.age]
    if args.filter_regex:
        filter_regexes = [re.compile(regex) for regex in args.filter_regex]
        data = [
            entry
            for entry in data
            if any(regex.search(entry["v"]["source"]) for regex in filter_regexes)
        ]
    return data
...
def _get_file_size(args, stats, input_data: Dict) -> Optional[int]:
    ...
    """
    try:
        input_data["source_size"] = os.path.getsize(input_data["v"]["source"])
        return input_data["source_size"]
    except OSError:
        if args.verbose:
            LOGGER.warning("File '%s' not found!", input_data["v"]["source"])
...
def _partial_read_stats(
    args: argparse.Namespace, stats: Dict, input_data: Dict,
):
    ...
    """
    if args.verbose:
        percent = (input_data["v"]["offset"] / input_data["source_size"]) * 100.0
        LOGGER.info(
            "Partially read file (%0.2f%%): %s  [offset=%s, size=%s]",
            percent,
            input_data["v"]["source"],
            input_data["v"]["offset"],
    ...
    if os.path.exists(args.registry_file):
        ...
        for input_data in registry_data:
            ...
            if source_size is None:
                continue
            if input_data["v"]["offset"] == source_size:
                if args.move:
                    _move_file(args, input_data["v"]["source"])
                elif args.delete:
                    _delete_file(args, input_data["v"]["source"])
                elif args.verbose:
                    LOGGER.info("Fully read file (100%%): %s", input_data["v"]["source"])
                if args.show_summary:
        ...
        LOGGER.fatal(
            "Registry file missing. Filebeat may not have created it "
